...
def prepare_engine_data():
    ...
    for t in teachers_list:
        ...
        if str(t_allowed_str).lower() == "all" or str(t_allowed_str) == "":
            ...
            allowed_names = [x.strip().lower() for x in str(t_allowed_str).split(",")]
            actual_classes = [c for c in classes_list if c.lower() in allowed_names]
...
with tab5:
    ...
    with col_chat:
        ...
        with st.form("chat_input_form", clear_on_submit=True):
            prompt = st.text_area(label="Paste Data Here:", height=80)
    ...
    with st.form("ai3_form", clear_on_submit=True):
        ai3_prompt = st.text_area(label="AI-3 se baat karein (Ya upar wala 'Auto-Analyze' button dabayein):", height=80)
    ...
    if trigger_prompt:
        if not nvidia_api_key:
            ...
            st.session_state.ai3_messages.append({
                "role": "user",
