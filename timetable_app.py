...
def generate_timetable_ai3(live_data_str, valid_periods_count):
    ...
    response = requests.post(url, headers=headers, json=payload, timeout=300)
    if response.status_code != 200:
        raise Exception(f"AI 3 Generator Error: {response.text}")
    return response.json()["choices"][0]["message"]["content"]
...
if "ai3_messages" not in st.session_state: 
    st.session_state.ai3_messages = [{
        "role": "assistant",
        "content": "Namaste Sir! Main AI-3 (The Fixer & Generator) hoon. Agar aapka timetable engine se fail ho raha hai, toh AI-3 se generate karwayein!"
    }]
