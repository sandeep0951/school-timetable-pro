...
with tab1:
  ...
  with col1:
    st.number_input(
        "1. Working Days", min_value=1, max_value=7, key="working_days"
    )
    st.number_input(
        "2. Periods per Day", min_value=1, max_value=20, key="periods_per_day"
    )
    st.number_input(
        "Lunch Break AFTER period?", min_value=1, max_value=15, key="break_at"
    )
    st.checkbox("4. Saturday Half-Day?", key="saturday_half_day")
  with col2:
      ...
      if not nvidia_api_key:
        ...
        if len(full_input.strip()) < 10:
          ...
          with st.spinner(
            ...
            try:
              # Triple-quoted raw string - kabhi quote wrap nahi hoga
              sys_p = r"""You are a strict JSON data extractor. Extract timetable parameters from the text and output ONLY RAW JSON matching this exact schema:
{
  ...
  "fixed_rules": ["Rule 1"]
              ...
              if data:
                ...
                # 6. Reset manual grids & bump sync_id to force widget refresh across tabs
                st.session_state.manual_grids = {}
