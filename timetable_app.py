...
# AI 3: Timetable Generator (Fallback)
def generate_timetable_ai3(live_data_str, valid_periods_count):
    url = "https://integrate.api.nvidia.com/v1/chat/completions"
    key = nvidia_api_key.strip() if nvidia_api_key else ""
    headers = {"Authorization": f"Bearer {key}", "Content-Type": "application/json"}
    system_prompt = (
        "You are an expert Timetable Generator AI. "
        "Generate a complete conflict-free weekly timetable using ONLY the provided classes, teachers, and rules.\n"
        f"CRITICAL INSTRUCTIONS:\n"
        f"1. For each class, provide an array of EXACTLY {valid_periods_count} period slots.\n"
        "2. Format each slot as 'Subject (Teacher Name)'. If no teacher is assigned, write 'Free Period'.\n"
        "3. No teacher should be double-booked at the same slot index across different classes.\n"
        "4. Output ONLY RAW JSON in this format:\n"
        "```json\n"
        "{\n"
        '  "timetable": {\n'
        '    "1-A": ["Maths (Rahul Jain)", "English (Amit Sharma)"],\n'
        '    "1-B": ["English (Amit Sharma)", "Maths (Rahul Jain)"]\n'
        "  }\n"
        "}\n"
        "```"
    )
    payload = {
        ...
        "messages": [
            {"role": "system", "content": system_prompt},
            {"role": "user", "content": f"LIVE APP DATA:\n{live_data_str}"}
        ],
        "temperature": 0.1,
        "max_tokens": 4096
    }
...
if "ai3_messages" not in st.session_state: 
    st.session_state.ai3_messages = [{"role": "assistant", "content": "Namaste Sir! Main AI-3 hoon. Agar engine se fail ho, toh AI-3 se generate karwayein!"}]
            ...
            if st.button("🚀 Run Python Engine", use_container_width=True):
                with st.spinner("Python Engine is working..."):
                    ...
                    if solve_custom(0, 0):
                        ...
                        st.warning("⚠️ Python Engine failed. Use AI-3 for generation or diagnosis.")
                ...
                with st.spinner("Advanced OR-Tools Engine working..."):
                    if not ORTOLS_AVAILABLE:
                        ...
                        if status in [cp_model.OPTIMAL, cp_model.FEASIBLE]:
                                        ...
                                        if solver.Value(x[c][p][r_idx]) == 1:
                                            ...
                                            for idx, label in enumerate(p_labels):
                                                if not "LUNCH" in label:
                                                    p_label_idx += 1
                                                    if p_label_idx == p - 1:
                                                        ortools_tt[c][idx] = "---" if req[0] == "-" else f"{req} ({req[0]})"
                                                        break
                            ...
                            st.error("❌ OR-Tools Failed. Absolute Deadlock. Use '🤖 AI-3 Generator (Fallback)' button!")

        with col_e3:
            if st.button("🤖 AI-3 Generator (Fallback)", use_container_width=True):
                if not nvidia_api_key:
                    st.error("❌ Key Missing!")
                else:
                    with st.spinner("AI-3 is generating complete timetable from tabs data..."):
                        try:
                            c_list, p_labels, v_periods, _, _, _, _ = prepare_engine_data()
                            live_data = {
                                "working_days": st.session_state.working_days,
                                "periods_per_day": st.session_state.periods_per_day,
                                "classes": st.session_state.classes_df["Class Name"].dropna().tolist(),
                                "teachers": st.session_state.teachers_df.to_dict("records"),
                                "rules": st.session_state.rules_df["Rule"].dropna().tolist(),
                                "valid_periods_count": len(v_periods)
                            }
                            live_data_str = json.dumps(live_data)
                            
                            ai_output = generate_timetable_ai3(live_data_str, len(v_periods))
                            start_idx = ai_output.find("{")
                            end_idx = ai_output.rfind("}")
                            clean_output = ai_output[start_idx:end_idx+1] if start_idx != -1 and end_idx != -1 else ai_output
                            clean_output = re.sub(r",\s*}", "}", clean_output)
                            clean_output = re.sub(r",\s*]", "]", clean_output)
                            
                            tt_json = json.loads(clean_output)
                            tt_data = tt_json.get("timetable", {})
                            ...
                            for c in c_list:
                                ...
                                for label in p_labels:
                                    if "LUNCH" in label:
                                        col_slots.append("LUNCH / BREAK")
                                    else:
                                        if slot_idx < len(slots):
                                            col_slots.append(slots[slot_idx])
                                            slot_idx += 1
                                        else:
                                            col_slots.append("---")
                                final_tt[c] = col_slots
                                
                            df_ai = pd.DataFrame(final_tt)
                            df_ai.insert(0, "Day / Period", p_labels)
                            st.success("🤖 AI-3 Timetable Generation Success! Check below:")
                            st.dataframe(df_ai, use_container_width=True, hide_index=True)
                        except Exception as e:
                            st.error(f"❌ AI-3 Generator Error: {e}")
        ...
        if not nvidia_api_key:
            ...
            with st.spinner("AI-3 data analyze kar raha hai..."):
                try:
                    ...
                    reply = chat_ai3(trigger_prompt, live_data_str, [m for m in st.session_state.ai3_messages if m.get("role") != "system"])
