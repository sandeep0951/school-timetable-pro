    ...
    with col_chat:
        ...
        if "chat_messages" not in st.session_state:
            st.session_state.chat_messages = [
                {
                    "role": "system", 
                    "content": '''You are Sandeep's strict Timetable Data Collector. 
    CRITICAL RULES:
        ...
        if st.button("🔄 1. Sync AI Rules from Chat", use_container_width=True):
            if not client:
                ...
                with st.spinner("Translating Chat History into UI Data..."):
                    ...
                    extraction_prompt = '''You are a strict data parsing tool. Extract the finalized timetable data into JSON format.
